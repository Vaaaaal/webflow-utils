...
|---|---|---|---|
| `data-combine` | nom du combo (string libre, ex. `featured`) | ✅ | — |
| `data-combine-sort-type` | `date` / `number` / `string` | ❌ | pas de tri |
| `data-combine-sort-dir` | `asc` / `desc` | ❌ | `asc` |
| `data-combine-limit` | entier (ex. `6`) | ❌ | pas de limite |

### Sur la liste source (Collection List Wrapper #2)

| Attribut | Valeurs | Obligatoire |
|---|---|---|
| `data-combine-source` | même nom que `data-combine` de la cible | ✅ |

### Sur chaque item (dans les deux collections)

| Attribut | Valeurs | Obligatoire |
|---|---|---|
| `data-sort-value` | valeur à utiliser pour le tri, bindée depuis un champ CMS | ✅ si tri activé |
...
<div class="w-dyn-list"
  ...
  <div role="list" class="w-dyn-items">
    <div role="listitem" class="w-dyn-item" data-sort-value="2025-03-15">
      <!-- contenu de l'item -->
    </div>
  </div>
</div>
...
```html
<div class="w-dyn-list" data-combine="featured">…</div>
<div class="w-dyn-list" data-combine-source="featured">…</div>

<div class="w-dyn-list" data-combine="latest">…</div>
<div class="w-dyn-list" data-combine-source="latest">…</div>
```
...
| `data-combine-sort-type` absent | Pas de tri, concaténation simple (cible d'abord, puis source) |
| `data-combine-limit` absent | Tous les items sont conservés |
| Item sans `data-sort-value` (avec tri activé) | Renvoyé en fin de liste |
...
1. Le wrapper cible a-t-il bien `data-combine="…"` ?
2. Le wrapper source a-t-il bien `data-combine-source="…"` avec le **même** nom (sensible à la casse) ?
3. Chaque item a-t-il un `data-sort-value` non vide (si tri activé) ? Inspecter dans la console.
...
- **v1.0.0** — Version initiale : fusion, tri (date/number/string), limite, support multi-combos.
